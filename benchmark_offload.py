...
def single_run(model_cfg: dict, run_index: int, seed: int,
               run_dir: Path) -> dict:
    """
    Execute one full lifecycle: monitor starts → model loads → image generated
    ...
    pytorch_allocated_peak_mb = (
        torch.cuda.max_memory_allocated() / 1024 ** 2
        if torch.cuda.is_available() else 0.0
    )
    stamp('gen_end')

    img.save(run_dir / f"run_{run_index:02d}.png")
    ...
    return {
        ...
        'vram_allocated_pytorch_peak_mb': pytorch_allocated_peak_mb,
...
def _worker_mode(rid: str, run_index: int, output_dir: Path):
    ...
    print(f"  load={r['load_time_s']:.1f}s  encode={r['encode_time_s']:.1f}s  "
          f"gen={r['gen_time_s']:.1f}s  "
...
def main():
    ...
    # Collect scalars written by each worker
    ...
    # ── Summary CSV: mean ± std per model config ──────────────────────────
    numeric_cols = [
        'load_time_s', 'encode_time_s', 'gen_time_s', 'total_time_s',
